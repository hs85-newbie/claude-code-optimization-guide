...
| 메모리 압력 | `memory_pressure \| awk '/pressure/ {print}'` | `Normal` = 여유 |
...
check_load() {
  ...
  # 메모리 압력
  local mem_pressure
  mem_pressure=$(memory_pressure 2>/dev/null | awk '/System-wide memory free percentage/ {gsub("%",""); print $NF}')
  mem_pressure="${mem_pressure:-50}"

  # 판정: CPU 여유 + 부하 낮음 + 메모리 여유
  awk -v i="$idle_int" -v t="$THRESHOLD_CPU_IDLE" -v l="$load1" -v ll="$LOAD_LIMIT" -v m="$mem_pressure" '
    BEGIN {
      if (i >= t && l <= ll && m >= 20) { print "OK"; exit 0 }
      else { printf "BUSY idle=%s%% load=%s mem_free=%s%%\n", i, l, m; exit 1 }
    }'
}
